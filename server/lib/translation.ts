...
const stylePrompts: Record<string, string> = {
  standard:
    "Translate this text maintaining a neutral, accurate style that preserves the meaning and tone of the original.",
  literal:
    "Translate this text as literally as possible, staying very close to the original wording while ensuring grammatical correctness.",
  technical:
    "Translate this text with precise technical terminology, maintaining all specialized terms and concepts.",
  literary:
    "Translate this text with literary flair, preserving metaphors, idioms, and stylistic elements while adapting them naturally to the target language.",
  coloquial:
    "Translate this text into a casual, conversational style using common expressions and everyday language in the target language.",
};
...
export async function translateText(
  ...
  try {
    ...
    if (TRANSLATION_PROVIDER === "GEMINI") {
      if (!genAI) {
        throw new Error(
          "Gemini API client not initialized. Check API key and provider selection.",
        );
      }
      console.log(
        `[translateText] Starting HTML translation with Gemini. Target: ${targetLang}, Style: ${style}. HTML length: ${htmlContent.length}`,
      );
      ...
      if (!openai) {
        throw new Error(
          "OpenAI API client not initialized. Check API key and provider selection.",
        );
      }
      console.log(
        `[translateText] Starting HTML translation with OpenAI. Target: ${targetLang}, Style: ${style}. HTML length: ${htmlContent.length}`,
      );
      ...
      throw new Error(
        `Invalid TRANSLATION_PROVIDER configured: ${TRANSLATION_PROVIDER}`,
      );
    }

    console.log(
      `[translateText] Translation successful. Output length: ${translatedHtml.length}`,
    );
    ...
    throw new Error(
      `Translation failed (${TRANSLATION_PROVIDER}): ${errorMessage}`,
    );
  }
}
